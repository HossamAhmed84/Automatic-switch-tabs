...
email_to = {'test1':'hossam.ahmed@sig.biz',
            'prod_fillerBD':'hossam.ahmed@sig.biz; ana.sedarati@sig.biz;mostafa.shalaby@sig.biz; waleed.radwan@sig.biz; abdelouahed.benbelgacem@sig.biz', 
            'prod_Track_BD':'hossam.ahmed@sig.biz; ana.sedarati@sig.biz;mostafa.shalaby@sig.biz; waleed.radwan@sig.biz; abdelouahed.benbelgacem@sig.biz'}
email_cc= 'hossam.ahmed@sig.biz'
email_subject = {'fillerBD':'Breakdown Automated Notification for ', 'Track-Off':'Track-Off Automated Notification for '}
email_subject1 = 'Track-Off Automated Notification for '
...
# function to send an email with the given message    
def send_emails_filler(customerName, lineName,machineName,filler_Type,filler_Speed,filler_SW, duration, timeUpdate, LPO, Operator_Name, Operator_LASTENTRY, Product_Name, Product_LASTENTRY):
    ol = win32com.client.Dispatch('Outlook.Application')
    # size of the new email
    olmailitem = 0x0
    newmail = ol.CreateItem(olmailitem)
    fullSubject=' '.join ([email_subject['fillerBD'], customerName, 'Line_', lineName])
    newmail.Subject = fullSubject
    newmail.To = email_to['test1']
    newmail.CC = email_cc
...
# function to send an email with the given message    
def send_emails_track(customerName, lineName,machineName,filler_Type,filler_Speed,filler_SW, duration, timeUpdate, LPO, GoodPacksOut, Operator_Name, Operator_LASTENTRY, Product_Name, Product_LASTENTRY):
    ol = win32com.client.Dispatch('Outlook.Application')
    # size of the new email
    olmailitem = 0x0
    newmail = ol.CreateItem(olmailitem)
    fullSubject=' '.join ([email_subject['Track-Off'], customerName, 'Line_', lineName])
    newmail.Subject = fullSubject
    newmail.To = email_to['test1']
    newmail.CC = email_cc
    newmail.Body ="Test should not appear"
    ...
    html_body = f"""\
        <html>
            <body>
                ...
                <p> This message to inform you that your Line_ <strong> {lineName} </strong> {filler_Type} filler number <strong>{machineName} </strong> has track/s switched off <strong> {duration} </strong> minutes.</p>
                <p>This is RC automated montoring system running over all <strong> {customerName}'s</strong> Lines and this notification about status update as of <strong>{timeUpdate}</strong>.</p>
                <p>The Total number of good packs produced since last two hour are <strong> {GoodPacksOut} Packs </strong>. </p>
                <p>The Lost Production Opportunity so far are <strong> {LPO} Packs </strong>. </p>
                <p> The current running Product Name as per HMI : <strong> {Product_Name} </strong>. </p>
                <p> The current User login on HMI Name is : <strong> {Operator_Name} </strong>. his contact is : <a href=“tel:971-588-224778”>971-588-224778</a></p>
                <p>Best regards,</p>
...
while True:
    ...
    for db in databases:
        ...
        # filter out
        filter_status = []
        #Filter to capture only rows where the datetime value is older than 120 minutes and filler complete only
        filler_status = []
        # filter out rows where the datetime value is older than 120 minutes and in track
        track_status = []
        #Rows for packs counters
        packs_rows= []
        #print('Database1')
        try:
            ...
            ##########
            #PACKS without spacific time
            #sql1 = "select facts.compatibility_ref,sum(facts.newvalue) sum_packages,stat_machine_ref, mach.prlname,comp.strname from sigfactscounter facts left join stat_machine mach on stat_machine_ref = mach.id left join def_compatibility comp on facts.compatibility_ref = comp.id where timestampstart between TO_TIMESTAMP('2023-02-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS') and TO_TIMESTAMP('2023-02-02 00:00:00', 'YYYY-MM-DD HH24:MI:SS') and compatibility_ref in (600,601,602,603) group by facts.compatibility_ref, stat_machine_ref, prlname, comp.strname"
            #sql1 = "select facts.compatibility_ref,sum(facts.newvalue) sum_packages,stat_machine_ref, mach.prlname,comp.strname from sigfactscounter facts left join stat_machine mach on stat_machine_ref = mach.id left join def_compatibility comp on facts.compatibility_ref = comp.id where timestampstart between TO_TIMESTAMP('2023-02-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS') and TO_TIMESTAMP('2023-02-02 00:00:00', 'YYYY-MM-DD HH24:MI:SS') and compatibility_ref in (600,601,602,603) group by facts.compatibility_ref, stat_machine_ref, prlname, comp.strname"
            sql2 = "select facts.compatibility_ref,sum(facts.newvalue) sum_packages,stat_machine_ref, mach.prlname,comp.strname from sigfactscounter facts left join stat_machine mach on stat_machine_ref = mach.id left join def_compatibility comp on facts.compatibility_ref = comp.id where timestampstart between :two_hours_ago_var and :two_minutes_ago_var and compatibility_ref in (600,601,602,603) group by facts.compatibility_ref, stat_machine_ref, prlname, comp.strname"
            #PACKS with spacific time between two hours before and now
            sql1 = "select facts.compatibility_ref,sum(facts.newvalue) sum_packages,stat_machine_ref, mach.prlname,comp.strname from sigfactscounter facts left join stat_machine mach on stat_machine_ref = mach.id left join def_compatibility comp on facts.compatibility_ref = comp.id where timestampstart between :two_hours_ago_var and :two_minutes_ago_var and compatibility_ref in (600,601,602,603) and mach.prlname in(:line_var) group by facts.compatibility_ref, stat_machine_ref, prlname, comp.strname"
            #Get all Packs for all lines
            #cursor.execute(sql2,  two_hours_ago_var=two_hours_ago, two_minutes_ago_var=two_minutes_ago)
            #packs_rows= cursor.fetchall()
            #print(packs_rows)
        ...
        if len(filter_status) == 0:
            ...
            write_logs(log_file_path, message)
        elif len(filter_status) > 0:
            message = ''
            for row in filter_status:
                #########################
                #check customer name
                try:
                ...
                line= row[0]
                print(line)
                cursor.execute(sql1, two_hours_ago_var=two_hours_ago, two_minutes_ago_var=two_minutes_ago, line_var=line)
                packs_rows= cursor.fetchall()
                print(packs_rows)
                if len(packs_rows) >0:
                    print(line + " has been produced packs last two hr so it is track issue")
                    message=  line + " has been produced packs last two hr so it is track issue"
                    write_logs(log_file_path, message)
                    for row in packs_rows:
                        if row[4] == 'CDAGoodPacksOut':
                            GoodPacksOut = int(row[1])
                            try:
                                LPO = LPO - GoodPacksOut
                            except:
                                LPO = 'LPO not defined'
                        
                    send_emails_track(customerName, lineName,machineName,filler_Type,filler_Speed,filler_SW, duration, two_minutes_ago_test, LPO, GoodPacksOut, Operator_Name, Operator_LASTENTRY, Product_Name, Product_LASTENTRY)
                    message= "Email notification for track-off has been send for " + lineName
                    write_logs(log_file_path, message)
                else:
                    print(line + " has been not produced any packs since last two hr so it is filler Breakdown")
                    message=  line + " has been not produced any packs since last two hr so it is filler Breakdown"
                    write_logs(log_file_path, message)
                    send_emails_filler(customerName, lineName,machineName,filler_Type,filler_Speed,filler_SW, duration, two_minutes_ago_test, LPO, Operator_Name, Operator_LASTENTRY, Product_Name, Product_LASTENTRY)
                    message= "Email notification for filler breackdown has been send for " + lineName
                    write_logs(log_file_path, message)
                    filler_status.append(row)
                    
                """"for row in filler_status:
                    if row[0] == machineName:
                        send_emails_filler(customerName, lineName,machineName,filler_Type,filler_Speed,filler_SW, duration, two_minutes_ago_test, LPO, Operator_Name, Operator_LASTENTRY, Product_Name, Product_LASTENTRY)
                        message= "Email notification for filler breackdown has been send for " + lineName
                        write_logs(log_file_path, message)
                for row in track_status:
                    if row[0] == machineName:
                        send_emails_track(customerName, lineName,machineName,filler_Type,filler_Speed,filler_SW, duration, two_minutes_ago_test, LPO, Operator_Name, Operator_LASTENTRY, Product_Name, Product_LASTENTRY)
                        message= "Email notification for track-off has been send for " + lineName
                        write_logs(log_file_path, message)"""
        if notConnected !=1:    
            connection.close()
